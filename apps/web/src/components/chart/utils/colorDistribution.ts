...
export const getDistributedColor = (index: number, totalItems?: number): string => {
    ...
    if (totalItems) {
        ...
        if (totalItems === 3) {
            ...
            return schemes[optimalIndices[index % 3] as number] as string;
        }

        // Special case for 2 items - use blue and orange (complementary colors)
        if (totalItems === 2) {
            const optimalIndices = [0, 6]; // blue, orange
            return schemes[optimalIndices[index % 2] as number] as string;
        }
        ...
        if (totalItems === 4) {
            ...
            return schemes[optimalIndices[index % 4] as number] as string;
        }
        ...
        if (totalItems > 1) {
            ...
            return schemes[(index * step + offset) % totalColors] as string;
        }
    }
    ...
    return schemes[colorIndex] as string;
};